 ...
 for num_client in $num_client_list; do
         for noniid_ratio in $non_iid_list; do
                 for cutlayer in $cutlayer_list; do
                         output_dir="./outputs/monacosfl${moco_version}_${arch}_${dataset}_cut${cutlayer}_bnl${bottleneck_option}_client${num_client}_nonIID${noniid_ratio}"
                         python run_sflmoco.py --num_client ${num_client} --lr ${lr} --cutlayer ${cutlayer} --num_epoch ${num_epoch}\
                                 ...
                                 --divergence_measure --seed 1138  --fedavg-momentum # disable fedavg-momentum for plain mocosfl
                 done
         done
 done
...
#        for noniid_ratio in $non_iid_list; do
#                for cutlayer in $cutlayer_list; do
#                        output_dir="./outputs/r-mocosfl${moco_version}_${arch}_${dataset}_cut${cutlayer}_bnl${bottleneck_option}_client${num_client}_nonIID${noniid_ratio}_af${avg_freq}_fam" #da${divaware_lambda}"
...
#                                --client_sample_ratio ${client_sample_ratio} --avg_freq ${avg_freq} --div_lambda ${divaware_lambda} \
#                                --divergence_measure --seed 1239 --fedavg-momentum #--gmatching oracle-most-similar --gmatching-ngrads 19
#
#                done
#        done
#done
