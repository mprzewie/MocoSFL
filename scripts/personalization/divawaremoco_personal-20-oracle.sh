...
QMATCHING="oracle-most-similar"
QMATCHING_NQUEUES=19
GMATCHING="oracle-most-similar"
GMATCHING_NGRADS=0
...
constant_args="--num_client ${num_client} --lr ${lr} --num_epoch ${num_epoch} --noniid_ratio ${noniid_ratio}  --moco_version ${moco_version} \
  ...
  --auto_adjust --divergence_measure --K $K --qmatching ${QMATCHING} --qmatching-nqueues ${QMATCHING_NQUEUES} --gmatching ${GMATCHING} --gmatching-ngrads ${GMATCHING_NGRADS} --batch_size ${BS}"
...
COMMON_SUFFIX="bs${BS}-qm-omost${QMATCHING_NQUEUES}-gm-omost${GMATCHING_NGRADS}"
...
#  output_dir="./outputs/${prefix}_cut${cutlayer}_no-ft-sharing-${COMMON_SUFFIX}"
...
# python run_sflmoco.py  $constant_args --cutlayer ${cutlayer} --output_dir ${output_dir} --div_lambda $DIV_LAMBDA --div_layerwise $DIVAWARE_LAYERWISE --divergence_aware
 ...
 output_dir="./outputs/${prefix}_cut${cutlayer}_div-aware_no-ft-sharing--${COMMON_SUFFIX}"
